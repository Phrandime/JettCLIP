        ...
        **kwargs,
        ...
        if self.mha_type == 1:
            ...
            x = self.pre_norm_mha[1](
                ...
                # need_weights=False, # not
                # *args, #not, may be not need 
                # **kwargs,#not, may be not need
            )  # mha
        else:
            res = x
            x = self.pre_norm_mha[0](x)  # norm
            ...
            x_prev = x_prev if x_prev==None else x_prev.to(torch.float16)
            key_padding_mask = key_padding_mask if key_padding_mask==None else key_padding_mask.to(torch.float16)
            x = self.pre_norm_mha[1](
                # x_q=x, # not
                x = x.to(torch.float16), # flash
                x_kv=x_prev,
                key_padding_mask=key_padding_mask,
                # attn_mask=attn_mask, # not
                # need_weights=False, # not
